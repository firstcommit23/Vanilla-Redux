...
var store = (0, _redux.createStore)(reducer); // 상태가 업데이트 될 때마다 호출
...
render(); // 구독하기
// react 프로젝트에서는 이 함수를 직접 사용하지 않음. react-redux라는 라이브러리가 이 작업을 대신 해줌.

store.subscribe(render); // 액션 발생시키기(디스패치)
// 이 함수가 호출되면 스토어는 리듀서를 실행시켜 새로운 상태를 만듬.
